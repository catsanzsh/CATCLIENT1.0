...
def setup_tlauncher_cosmetics(account, status_callback=None, ssl_verify=False):
    if status_callback: status_callback(f"Setting up TLauncher cosmetics for {account['username']}...")
    ...
    if cape_path:
        ...
        os.makedirs(os.path.dirname(cape_asset_path), exist_ok=True)
        shutil.copy(cape_path, cape_asset_path)
    if status_callback: status_callback(f"TLauncher cosmetics setup complete for {account['username']}")
