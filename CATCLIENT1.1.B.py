...
def add_account(acc_type, email_username, password_token=None):
    if not email_username:
        print("No username provided for account creation!")
        return
...
def download_file(url, dest_path, description="file", ssl_verify=False):
    ...
    try:
        ...
        raise Exception(f"Failed to download {description}: HTTP Error {e.code} - {e.reason}")
    except Exception as e:
        print(f"Failed to download {description} from {url}: {e}")
        raise Exception(f"Failed to download {description}: {e}")
...
def setup_lunar_client(version_id, account, status_callback=None, ssl_verify=False):
    ...
    if not os.path.exists(settings_path):
        with open(settings_path, 'w') as f:
            json.dump({"gameDir": mc_dir, "jreDir": os.path.join(lunar_dir, "jre"), "lastVersion": version_id, "offline": False}, f, indent=4)
...
def launch_game(version_id, account, ram_mb=1024, java_path="java", game_dir=None, server_ip=None, port=None, 
    ...
    if not account.get("username"):
        error_msg = "Auth username not found in account!"
        print(error_msg)
        if status_callback: status_callback(error_msg, "#ff0000")
        raise ValueError(error_msg)

    print(f"Launching with username: {account['username']}")
    ...
    try:
        process = subprocess.Popen(command, cwd=effective_game_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
class M1LauncherApp:
    def __init__(self, root):
        ...
        self.lunar_client_var = tk.BooleanVar(value=True)
    ...
    def refresh_account_list(self):
        if not accounts:
            add_account("offline", "Player")  # Add default account if none exist
        self.account_combo['values'] = [f"{acc['username']}" for acc in accounts]
        self.account_combo.current(0)  # Always select first account
    ...
    def on_launch(self):
        ...
        if account_index == -1:
            messagebox.showerror("Error", "No account selected! This shouldn’t happen with default Player.")
            return
